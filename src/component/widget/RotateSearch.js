...
let childButtonIcons = ['user', 'sound', 'file-text', 'bell', 'smile', 'bolt', 'ban', 'code'];
...
class RotateSearch extends React.Component {
  constructor(props) {
    super(props);
  ...
  renderChildButtons() {
    ...
    const targetButtonStylesInitObject = _.range(NUM_CHILDREN).map(i => {
    ...
    const targetButtonStyles = _.range(NUM_CHILDREN).map(i => {
				...
				{interpolatedStyles =>
					<div>
						{interpolatedStyles.map(({height, left, rotate, scale, top, width}, index) =>
							...
							>
                                <Icon type={childButtonIcons[index]} style={{ fontSize: '16px', color: '#08c' }} />
							</div>
						)}
					</div>
				}
				...
				<Motion style={mainButtonRotation}>
					{({rotate}) =>
						<div
                            ...
                            <Icon type="search" theme="outlined" style={{ fontSize: '26px' }} />
						</div>
					}
				</Motion>
			</div>
